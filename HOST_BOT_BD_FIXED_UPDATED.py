...
def bc_type_inline():
    ...
    kb.row(types.InlineKeyboardButton("🎤 Audio",callback_data="bc_audio"),
           types.InlineKeyboardButton("🔗 Link",callback_data="bc_link"))
    kb.add(types.InlineKeyboardButton("❌ Cancel",callback_data="close_panel"))
    return kb
...
@bot.message_handler(commands=["buttonbroadcast"])
def cmd_buttonbroadcast(msg):
    """Broadcast a URL to all non-banned users with an Open Link button."""
    uid=msg.from_user.id
    if not is_admin(uid): return
    parts=msg.text.split(maxsplit=1)
    if len(parts)<2 or not parts[1].strip():
        user_state[uid]="adm_broadcast_link"
        bot.send_message(uid,"🔗 ব্রডকাস্টের জন্য লিংক পাঠান:",reply_markup=cancel_only_kb("bn"))
        return
    link=parts[1].strip()
    if not (link.startswith("http://") or link.startswith("https://")):
        bot.send_message(uid,"❌ সঠিক লিংক দিন। http:// অথবা https:// দিয়ে শুরু হতে হবে।")
        return
    threading.Thread(target=do_broadcast,args=(uid,link,"all","link",link),daemon=True).start()
    bot.send_message(uid,"🔗 Link broadcast শুরু হয়েছে...",reply_markup=admin_bot_kb())
...
def handle_text(msg):
    try:
        ...
        if state=="adm_broadcast_link":
            link=text.strip()
            if not (link.startswith("http://") or link.startswith("https://")):
                bot.send_message(uid,"❌ সঠিক লিংক দিন। http:// অথবা https:// দিয়ে শুরু হতে হবে:",reply_markup=cancel_only_kb(lang)); return
            clear_state_keys(uid)
            threading.Thread(target=do_broadcast,args=(uid,link,"all","link",link),daemon=True).start()
            bot.send_message(uid,"🔗 Link broadcast শুরু হয়েছে...",reply_markup=admin_bot_kb()); return
        if state.startswith("tbc_target_active_"):
            target_id=int(state[len("tbc_target_active_"):])
            try:
                ssend(target_id,text)
                audit(uid,"TARGETED_TEXT",target_id,text[:80])
                bot.send_message(uid,f"✅ {target_id} কে Text পাঠানো হয়েছে।\nআরও Text/Image/Audio পাঠাতে পারেন।\n❌ বাতিল চাপলে Targeted Broadcast বন্ধ হবে।",reply_markup=cancel_only_kb(lang))
            except Exception as e:
                bot.send_message(uid,f"❌ {target_id} কে পাঠানো যায়নি: {e}",reply_markup=cancel_only_kb(lang))
            return
        ...
        if state.startswith("tbc_msg_uid_"):
            target_uid_str=state[len("tbc_msg_uid_"):]
            try:
                tid=int(target_uid_str)
                if not get_user(tid):
                    bot.send_message(uid,"❌ এই User ID পাওয়া যায়নি। আবার User ID দিন:",reply_markup=cancel_only_kb(lang))
                    user_state[uid]="tbc_userid_input"
                    return
                user_state[uid]=f"tbc_target_active_{tid}"
                ssend(tid,text)
                audit(uid,"TARGETED_TEXT",tid,text[:80])
                bot.send_message(uid,f"✅ {tid} কে Text পাঠানো হয়েছে।\n\n🎯 এখন Targeted Broadcast সক্রিয়।\n📝 Text / 🖼️ Image / 🎤 Audio পাঠালেই ওই ইউজারের কাছে যাবে।\n❌ বাতিল চাপলে বন্ধ হবে।",reply_markup=cancel_only_kb(lang))
            except ValueError:
                bot.send_message(uid,"❌ সঠিক User ID দিন:",reply_markup=cancel_only_kb(lang))
            except Exception as e:
                bot.send_message(uid,f"❌ Failed: {e}",reply_markup=cancel_only_kb(lang))
            return
...
def handle_photo(msg):
    ...
    if state.startswith("tbc_target_active_") and is_admin(uid):
        target_id=int(state[len("tbc_target_active_"):])
        try:
            bot.send_photo(target_id,msg.photo[-1].file_id,caption=msg.caption or "")
            audit(uid,"TARGETED_IMAGE",target_id,"photo")
            bot.send_message(uid,f"✅ {target_id} কে Image পাঠানো হয়েছে।",reply_markup=cancel_only_kb(lang))
        except Exception as e:
            bot.send_message(uid,f"❌ Image পাঠানো যায়নি: {e}",reply_markup=cancel_only_kb(lang))
        return
    if state=="await_support":
        clear_state_keys(uid)
...
def handle_voice(msg):
    ...
    if state.startswith("tbc_target_active_") and is_admin(uid):
        target_id=int(state[len("tbc_target_active_"):])
        try:
            bot.send_voice(target_id,msg.voice.file_id,caption=msg.caption or "")
            audit(uid,"TARGETED_AUDIO",target_id,"voice")
            bot.send_message(uid,f"✅ {target_id} কে Audio পাঠানো হয়েছে।",reply_markup=cancel_only_kb(lang))
        except Exception as e:
            bot.send_message(uid,f"❌ Audio পাঠানো যায়নি: {e}",reply_markup=cancel_only_kb(lang))
        return
    if state=="adm_broadcast_audio" and is_admin(uid):
        clear_state_keys(uid)
        threading.Thread(target=do_broadcast,args=(uid,msg.caption or "","all","voice",msg.voice.file_id),daemon=True).start()
        bot.send_message(uid,"🎤 Audio broadcast শুরু হয়েছে...",reply_markup=admin_bot_kb())
...
def handle_audio(msg):
    ...
    if state.startswith("tbc_target_active_") and is_admin(uid):
        target_id=int(state[len("tbc_target_active_"):])
        try:
            bot.send_audio(target_id,msg.audio.file_id,caption=msg.caption or "")
            audit(uid,"TARGETED_AUDIO",target_id,"audio")
            bot.send_message(uid,f"✅ {target_id} কে Audio পাঠানো হয়েছে।",reply_markup=cancel_only_kb(lang))
        except Exception as e:
            bot.send_message(uid,f"❌ Audio পাঠানো যায়নি: {e}",reply_markup=cancel_only_kb(lang))
        return
    if state=="adm_broadcast_audio" and is_admin(uid):
        clear_state_keys(uid)
        threading.Thread(target=do_broadcast,args=(uid,msg.caption or "","all","audio",msg.audio.file_id),daemon=True).start()
        bot.send_message(uid,"🎤 Audio broadcast শুরু হয়েছে...",reply_markup=admin_bot_kb())
...
def handle_callback(call):
    try:
        ...
        if data=="bc_audio" and is_admin(uid):
            user_state[uid]="adm_broadcast_audio"; sdel(call.message.chat.id,call.message.message_id)
            bot.send_message(uid,"🎤 Voice/Audio পাঠান:",reply_markup=cancel_only_kb("bn")); return
        if data=="bc_link" and is_admin(uid):
            user_state[uid]="adm_broadcast_link"; sdel(call.message.chat.id,call.message.message_id)
            bot.send_message(uid,"🔗 ব্রডকাস্টের জন্য লিংক পাঠান:",reply_markup=cancel_only_kb("bn")); return
    except Exception as e:
...
def do_broadcast(admin_uid,text,target,media_type,media_id):
    ...
    for u in users:
        try:
            ...
            elif media_type=="voice": bot.send_voice(u["user_id"],media_id,caption=text)
            elif media_type=="audio": bot.send_audio(u["user_id"],media_id,caption=text)
            elif media_type=="link":
                kb=types.InlineKeyboardMarkup()
                kb.add(types.InlineKeyboardButton("🔗 Open Link",url=media_id))
                bot.send_message(u["user_id"],media_id,reply_markup=kb)
